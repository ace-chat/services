...
func (t *MarketingGeneratorRequest) Generator(user model.User) serializer.Response {
	...
	if err != nil {
		...
		zap.L().Error("[Marketing] Get tone failure", zap.Error(err))
	...
	if t.BrandVoice != nil {
		...
		if err != nil {
			...
			zap.L().Error("[Marketing] Get brand voice failure", zap.Error(err))
			return serializer.DBError(err)
		}
		voice = v.Content
		voiceId = v.Id
	...
	if t.Region != nil {
		...
		if err != nil {
			...
			zap.L().Error("[Marketing] Get region failure", zap.Error(err))
			return serializer.DBError(err)
		}
		region = r.Country
		regionId = r.Id
	...
	if t.Gender != nil {
		...
		if err != nil {
			...
			zap.L().Error("[Marketing] Get gender failure", zap.Error(err))
			return serializer.DBError(err)
		}
		gender = g.Value
		genderId = g.Id
	...
	if err != nil {
		...
		zap.L().Error("[Marketing] Get language failure", zap.Error(err))
		return serializer.DBError(err)
	}
	...
	if err := tx.Model(&model.EmailAds{}).Create(&ads).Error; err != nil {
		zap.L().Error("[Marketing] Create email ads failure", zap.Error(err))
		tx.Rollback()
		return serializer.DBError(err)
	}
	...
	body, err := request.Client.Post(model.Url["generate_cold_marketing_email_content"])
	if err != nil {
	...
	if err := tx.Model(&model.EmailContent{}).Create(&content).Error; err != nil {
		zap.L().Error("[Marketing] Create email content failure", zap.Error(err))
		tx.Rollback()
		return serializer.DBError(err)
	}
