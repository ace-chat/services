...
type MarketingHistoryIdRequest struct {
	Id uint `form:"id" json:"id" binding:"required"`
}

func (m *MarketingHistoryIdRequest) GetMarketingContentById(user model.User) serializer.Response {
	...
	if err := cache.DB.Model(&model.EmailAds{}).Where("id = ?", content.AdsId).First(&email).Error; err != nil {
		...
		zap.L().Error("[Marketing] Get email ads failure", zap.Error(err))
		return serializer.DBError(err)
	}
